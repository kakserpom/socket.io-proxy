...
router.use(function (socket, args, next) {
    ...
    if (socket.ws == null || !socket.ws.connected) {
        ...
        console.log('C2S: Buffering ' + event);
        buffers[socket.id].push(args);
    } else {
        console.log('C2S: Sending ' + event);
...
io.on('connection', function (socket) {
    ...
    ws.on('connectFailed', function (error) {
        socket.log('Connect Error: ' + error.toString());
        socket.disconnect();
    });

    ws.on('connect', function (connection) {
        socket.log('connected to ' + argv['url']);
        connections[socket.id] = connection;
        if (buffers[socket.id] != null) {
            buffers[socket.id].forEach(function(item) {
                let event = item[0];
                let data = item[1];
                socket.log('C2S: Sending ' + event + ' from buffer');
        ...
        connection.on('error', function (error) {
            socket.disconnect();
            socket.log("Connection Error: " + error.toString());
        });
        connection.on('close', function () {
            socket.disconnect();
            socket.log('echo-protocol Connection Closed');
        });
        connection.on('message', function (message) {
            if (message.type === 'utf8') {
                var event = JSON.parse(message.utf8Data);
                socket.log('received', event);
                socket.emit.apply(socket, JSON.parse(message.utf8Data));
            }
        });
    });

    socket.log('connecting to ' + argv['url']);
